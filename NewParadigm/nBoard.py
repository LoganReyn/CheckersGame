...
from LogicHelp import (InvalidSelection, WhiteLost, BlackLost, _checkType, _rowColGen, userChoice)
...
##################################################
    ...
    def allMoves(self) -> dict[str, dict[str, tuple]]:
        theMoves = dict()
        for row, col in _rowColGen():
            currentSquare = self.getSquare((row, col))
            if currentSquare.isOccupied():
                currentPiece = currentSquare.getOccupant()
                if currentPiece.getTeam() == "b":
                    self.setSelection((row, col))
                    self.__gridMoves()
                    self.__removeSameTeamMoves()
                    cDict = self.getMoves()
                    if len(cDict) > 0:
                        theMoves[(row, col)] = cDict
        
        return theMoves
    
    def chooseMove(self, allMoves:dict, myMove):
        self.setMoves(allMoves[myMove])
